...
test.describe('Set of tests to verify functionality of manage users for Level 1 user', () => {
  ...
  test(
    ...
    async ({ apiClient, powerAppPages }) => {
      ...
      await test.step('Verify Add user, Search user and Manage court access buttons are visible', async () => {
        await expect(powerAppPages.manageUsersPage.$interactive.addUserButton).toBeVisible();
        await expect(powerAppPages.manageUsersPage.$interactive.searchUserButton).toBeVisible();
        await expect(powerAppPages.manageUsersPage.$interactive.manageCourtAccessButton).toBeVisible();
      });
    },
  );
});
